...
export class CharacterSheetManager {
    ...
    initializeSheetLogic() {
         const sheetRoot = this.dom.querySelector('#character-sheet');
    if (!sheetRoot) {
        console.warn('Character sheet root not found');
        return;
    }
  
    // If jQuery and Bootstrap are loaded, let Bootstrap initialize the tabs.
    // This will trigger the data-toggle="tab" logic already set on your links.
    if (window.jQuery) {
        jQuery(sheetRoot).find('#tabs a').on('click', function (e) {
            e.preventDefault();
            jQuery(this).tab('show');
        });
  
        // Optionally, ensure the default active tab is shown:
        const activeLink = sheetRoot.querySelector('#tabs a.active');
        if (activeLink) {
            jQuery(activeLink).tab('show');
        }
    }
  
    console.debug('Bootstrap tab functionality initialized');
