export class CharacterSheetManager {
    ...
    async init() {
        ...
        corners.forEach(corner => {
            ...
            grabber.classList.add('corner-grabber', corner);
            draggable.appendChild(grabber);
        });

        // Update draggable initialization to use all grabbers
        $(draggable).draggable({
            handle: '.drag-grabber, .corner-grabber',
            containment: 'window'
        });

// Initial size and position
        this.dom.style.width = '400px';
        this.dom.style.height = '600px';1      th1is.dom.style.position = 'fixed';
