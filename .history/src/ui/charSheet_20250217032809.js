export class CharacterSheetManager {
    ...
    async init() {
        ...
        if (!draggableTemplate) {
            console.warn('[CharSheet] Draggable template not found.');
            return;
        }
        ...
        if (!draggable || !closeButton) {
            console.warn('[CharSheet] Failed to find draggable or close button.');
            return;
        }
        ...
        scrollContainer.id = 'charSheetManagerContainer';
        draggable.appendChild(scrollContainer);

        movingDivs.appendChild(draggable);
        this.dom = draggable;

        // Remove jQuery UI draggable/resizable
        // $(draggable).draggable({...}).resizable({...});
        
        // Add ST's drag functionality
