export class CharacterSheetManager {
    ...
    initializeSheetLogic() {
        // Query for all .char-nav-tabs that have data-tab, then link them to .tab-pane areas
        const tabs = this.dom.querySelectorAll('.char-nav-tabs[data-tab]');
        const tabPanes = {
            'coretab': this.dom.querySelector('#tab-core'),
            'inventorytab': this.dom.querySelector('#tab-inventory'),
            'spellstab': this.dom.querySelector('#tab-spells'),
        };
        ...
        // (Optional) If you want a default active tab on initialization, you can pick one:
        // tabs[0]?.classList.add('active');
        // tabPanes['coretab']?.classList.add('active');
