...
export class CharacterSheetManager {
    constructor() { // The DOM node for the entire sheet UI 
    this.dom = null; this.isActive = false; }

async init() { // Load the HTML snippet for our extension (charSheet.html) 

    const response = await fetch('/scripts/extensions/third-party/SillyTavern-CharSheet/charSheet.html'); if (!response.ok) { console.warn('Failed to fetch charSheet.html'); return; } 
    ...
    const template = document.createRange() .createContextualFragment(await response.text()) .querySelector('#character-sheet');
        ...
        const pcAttackInputs = this.dom.querySelector('#pc-attack-inputs'); if (pcAttackInputs) { pcAttackInputs.addEventListener('change', () => this.updatePcAttackMacro()); pcAttackInputs.addEventListener('keyup', () => this.updatePcAttackMacro()); this.updatePcAttackMacro(); }
            ...
            updatePcAttackMacro() {
                const atkPrefix = this.dom.querySelector('#pc-atk-prefix')?.value || ''; const itemNum = this.dom.querySelector('#pc-item-num')?.value || '1'; let macroStr = Attack Macro for item ${ itemNum }; if (atkPrefix.trim() !== '') { macroStr += (${ atkPrefix }); }
                 // Display the result in the “pc-atk-macro” textarea this.dom.querySelector('#pc-atk-macro').value = macroStr; }
