export class CharacterSheetManager {
    ...
    setupTabs(container) {
        const tabsContainer = /** @type {HTMLElement} */ (
            container.querySelector('.toptabs')
        );
        ...
        tabsContainer.addEventListener('click', (e) => {
            ...
            const tabWrapper = target.closest('.tabwrapper');
            
            if (!tabWrapper) return;
            
            const tabId = tabWrapper.getAttribute('data-tab');
            if (!tabId) return;

            // Update tab wrappers
            container.querySelectorAll('.tabwrapper').forEach(tab => {
            ...
            const contentPane = /** @type {HTMLElement} */ (
