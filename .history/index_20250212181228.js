...
eventSource.on(event_types.CHAT_CHANGED, () => { if (context.extensionSettings[settingsKey].enabled) { initializeCharacterSheet(); // Example: show a small toast or log 
// 
...
eventSource.on(event_types.EXTENSION_UNLOADED, (extName) => { if (extName === EXTENSION_NAME) { // Hide or clean up the UI characterSheetManager?.hide(); const drawer = document.getElementById(${settingsKey}-drawer); if (drawer) { drawer.style.display = 'none'; } } }); })();