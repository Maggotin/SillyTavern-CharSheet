...
let inlineDrawer = document.getElementById(${settingsKey}-drawer); if (!inlineDrawer) { // Create container inlineDrawer = document.createElement('div'); inlineDrawer.id = ${settingsKey}-drawer; inlineDrawer.className = 'inline-drawer';
    // Header
...
} else { // If re-rendering, simply un-hide inlineDrawer.style.display = ''; }

// If enabled, load and show the sheet if (context.extensionSettings[settingsKey].enabled) { initializeCharacterSheet(); } }
...
// Entry point, after extension is loaded (function initExtension() { const context = getContext();

// Create or load extension settings if (!context.extensionSettings[settingsKey]) { context.extensionSettings[settingsKey] = structuredClone(defaultSettings); }
