...
import { getContext } from '../../../extensions.js'; import { CharacterSheetManager } from '../../../charSheet.js';

const settingsKey = 'SillyTavern-CharSheet';
const EXTENSION_NAME = 'Character Sheet D&D5e';

/**
 * @type {SillyTavernCustomChatWindowSettings}
 * @typedef {Object} SillyTavernCustomChatWindowSettings
...
(function initExtension() {
    ...
    // Listen for changes in the chat or other events 
    eventSource.on(event_types.CHAT_CHANGED, () => {
        if (context.extensionSettings[settingsKey].enabled) {
            initializeCharacterSheet(); // Example: show a small toast or log 
            // 
            toastr.info('Character Sheet loaded', EXTENSION_NAME);
        }
    });

    // Make our extension’s UI in the settings panel 
    renderExtensionSettings();

    // Listen for extension unloading 
    eventSource.on(event_types.EXTENSION_UNLOADED, (extName) => {
        if (extName === EXTENSION_NAME) {
            // Hide or clean up the UI 
            characterSheetManager?.hide(); const drawer = document.getElementById(${ settingsKey } - drawer); if (drawer) { drawer.style.display = 'none'; }
        }
    });
})();